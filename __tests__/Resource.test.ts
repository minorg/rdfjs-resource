...
describe("Resource", () => {
  ...
  describe("toList", () => {
    ...
    it("should read a list with one literal", ({ expect }) => {
      ...
      const list = resource
        ...
        .chain((_) => _.toList())
        .unsafeCoerce()
        .map((_) => _.toUnconstrainedString().unsafeCoerce());
  ...
  describe("value", () => {
    ...
    it("present", ({ expect }) => {
      expect(
        testResource
          .values(predicate)
          .find((value) => value.toUnconstrainedTerm().termType === "NamedNode")
          .unsafeCoerce()
          .toUnconstrainedIri()
          .toMaybe()
          .extract()?.value,
  ...
  describe("values", () => {
    it("every added value present", ({ expect }) => {
      ...
      for (const object of Object.values(objects)) {
        expect(
          values.find((value) => value.toUnconstrainedTerm().equals(object)),
        ).toBeDefined();
      }
    });
    ...
    it("named graph", ({ expect }) => {
      ...
      expect(
        actualValues[0].toUnconstrainedTerm().equals(literals.string),
      ).toStrictEqual(true);
    });
